//! The PPM engine and the minute-window ledger (`src/main/combat/procWindows.ts`).
//!
//! `proc_rate` carries three denominators, none hidden, each absent below its sample floor rather
//! than 0 — one proc in a two-second pull is not 30 ppm. `WindowAccum` is the wall-clock-minute
//! ledger the Tier-B counterfactual needs, plus the eligibility partition and the comparison itself.
//!
...
/// `inactive_swings × (with_count / active_swings)`.
///
...
pub struct RateInput {
    ...
    /// passing neither field, which is what the overall headline does — there the segment is the
    /// subject, not an assumption.
...
/// for that many swings — the only estimate available when `without_count == 0`, which is the case
/// the gate exists for.
...
/// rate-aware gate in `link_strength` and not mere 100% concentration. `damage` / `heal` are the
/// lane's whole totals, because `exclusive` means `without_count == 0`.
///

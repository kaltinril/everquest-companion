//! What makes committing `protocol/src/generated.rs` honest: the generator is re-run in memory and
//! its output compared to the bytes on disk, so a schema edit that lands without
//! `npm run gen:protocol` turns this red.
//!
//! It regenerates in full, unlike the TypeScript suite's check of the same file — that one can only
//! compare the header digest, because `npm test` may run where there is no Rust toolchain. Here the
//! compiler is present by definition, so the comparison is over every byte.
...
fn the_generated_file_carries_the_digest_of_the_schema_it_came_from() {
    // The cross-language half of the staleness check: `tests/protocolSchema.test.mts` recomputes the
    // same digest and looks for this same line, which is how the node suite can call the Rust
    // artifact stale without a Rust toolchain.
...
fn every_definition_in_the_schema_became_a_rust_type() {
    // The merge guard: the bundling step is mirrored in two languages, and a definition one merge
    // dropped would simply not be here.
    //
    // Two deliberate absences, the same defect twice — typify lowers a multi-type schema to an enum
    // whose number arm is `f64`, so a count comes back with a decimal point. `Cell` is replaced by
    // the hand-written `protocol::cell::Cell` and `ModuleState` by `serde_json::Value`. Both are
    // named here and in `tests/protocolSchema.test.mts`, so a third cannot appear silently.
    let files = protocol_codegen::read_schema_files(&schema_dir()).expect("the schema reads");
...
fn a_schema_edit_that_skips_the_generator_is_detectable() {
    // Proves the check itself works: perturb the schema text in memory and the digest must move.
    // Without this, a digest function returning a constant would pass every other assertion here.

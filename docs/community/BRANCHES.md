...
| `community/pr-47-hide-raid-targets` | [#47](https://github.com/jmoyers/everquest-companion/pull/47) | Bosses: hide a target from the Raid Targets roster and the tally, peek at hidden ones from the toolbar. Three commits on top: the PR's own e2e spec passes `max-depth`, its unit test no longer imports a helper JOS-499 retired, and the hide button sits at the art's bottom-right instead of over the tier chip. Conflicts with `pr-68` in two files; merged after it, rerere holds the union. |
